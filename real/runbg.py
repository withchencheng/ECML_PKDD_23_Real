...
def main():
    ...
    # 占用一块GPU的实验
    device='cuda:1'
    gpu_budget=5 # sst2: 最大用量 5197MiB

    logdir =home_dir+'data/AL/experiment/'+'tst_dev/' #500
    spidir =home_dir+'data/AL/experiment/'+'spi_dev/' # dir for sample info analysis
    config_pool=[]

    hp_agnews=HyperParam(task='agnews',seeds=[0,1], total_ntrain=1300, warm_size=100, rounds=8, ncentroids=[37, 22] ,unctth=[])
    hp_dbpedia=HyperParam(task='dbpedia',seeds=[0,1], total_ntrain=600, warm_size=70, rounds=8, ncentroids=[15], unctth=[])
    hp_qqp=HyperParam(task='qqp',seeds=[0,1], total_ntrain=6000, warm_size=400, rounds=8, ncentroids=[20,50] ,unctth=[])
    hp_sst=HyperParam(task='sst-2',seeds=[0,1], total_ntrain=800, warm_size=70, rounds=8, ncentroids=[46, 51], unctth=[])
    hp_pubmed=HyperParam(task='pubmed',seeds=[0,1], total_ntrain=1500, warm_size=200, rounds=8, ncentroids=[25] , unctth=[])
    hp=hp_sst

    
    al_method = 'random'
    seed=99
    for hp in [hp_sst, hp_qqp, hp_agnews, hp_dbpedia, hp_pubmed]:
        config=Config(hp.warm_size, hp.total_ntrain, hp.rounds, home_dir, logdir, spidir, hp.task, seed, device, al_method, ncentroids=None, unctth=-1)
