...
from tstrun.run_config_pseudo_err_1_4_1 import hp_agnews
...
def main():
    ...
    # hp_agnews=HyperParam(task='agnews',seeds=[0,1], total_ntrain=1300, warm_size=100, rounds=8, ncentroids=[10, 16, 20,25,30] ,unctth=[])
    # hp_agnews=HyperParam(task='agnews',seeds=[0,1], total_ntrain=1300, warm_size=100, rounds=8, ncentroids=[18, 22, 37, 40, 50, 60] ,unctth=[])
    # 20230131: 第二次调优所用的参数
    ...
    for al_method in al_methods:
        for nc in hp.ncentroids:
            for seed in hp.seeds:
                config=Config(hp.warm_size, hp.total_ntrain, hp.rounds, home_dir, logdir, spidir, hp.task, seed, device, al_method, ncentroids=nc, unctth=-1)
                config_pool.append(config)
    # ---------------------------------------------- tst17 ---------------------------------------------------
